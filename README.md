...
The playground includes presets for unauthenticated, OAuth, bearer-token, manual-client, and MCP Apps scenarios, including Excalidraw's hosted MCP App endpoint plus hosted remote MCP endpoints from Postman, PostHog, Webflow, Notion, and Atlassian. Remote presets use an app-owned transport proxy for MCP transport and OAuth background HTTP; OAuth state remains browser-owned.
